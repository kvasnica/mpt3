function run_lp_tests
%
%  RUN_LP_TESTS:   run tests for LP 
%  =================================
%     
%    
%    
...
%name = {'linprog', 'CDD.Criss-Cross', 'CDD.Dual-Simplex', 'CLP', 'glpk', 'nag', 'cplex', 'gurobi', 'sedumi'};
name = MPTOPTIONS.solvers_list.LP;
tol = 1e-5;
...
for i=1:numel(name)
    ...
    % testing cycling
    disp('Testing cycling');
    p = dir([d,'test_lp_cycling*']);
    for j=1:numel(p)
        try
            eval([strtok(p(j).name,'.'),'(''',name{i},''',',num2str(tol),');']);
            disp([p(j).name,'........................ ok']);
