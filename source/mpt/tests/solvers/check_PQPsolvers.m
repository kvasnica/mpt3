...
if nargin==0    
    s = '';
    for i=1:numel(MPTOPTIONS.solvers_list.parametric.QP)
        [worked, msg] = run_in_caller(sprintf('%s(MPTOPTIONS.solvers_list.parametric.QP{i});',fname));
        if ~worked
            s = [s,' ',MPTOPTIONS.solvers_list.parametric.QP{i}];
        end
    end
    if ~isempty(s)
