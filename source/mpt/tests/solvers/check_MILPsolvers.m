...
if nargin==0    
    s = '';
    for i=1:numel(MPTOPTIONS.solvers_list.MILP)
        [worked, msg] = run_in_caller(sprintf('%s(MPTOPTIONS.solvers_list.MILP{i},MPTOPTIONS.rel_tol);',fname));
        if ~worked
            s = [s,' ',MPTOPTIONS.solvers_list.MILP{i}];
        end
    end
    if ~isempty(s)
