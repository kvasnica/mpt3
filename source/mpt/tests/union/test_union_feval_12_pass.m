function test_union_feval_12_pass
% arrays of unions, single-valued functions, no tiebreak
...
[f, feasible] = U.forEach(@(u) u.feval(x, 'f'));
assert(isequal(size(f), [1 2]));
assert(isnan(f(1)));
assert(f(2)==f2(x));
assert(isequal(feasible, [false true]));
% third output is non-scalar
[worked, msg] = run_in_caller('[f,feasible,idx]=U.forEach(@(u) u.feval(x, ''f''));');
assert(~worked);
asserterrmsg(msg, 'Non-scalar in Uniform output');
% correct syntax
