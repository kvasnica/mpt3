function test_filter_setconstraint_01_pass

x = SystemSignal(2);
x.userData.kind = 'x';

% the filter should not be active by default
assert(~x.hasFilter('setConstraint'));

% add the filter
x.with('setConstraint');
assert(x.hasFilter('setConstraint'));

% the set must be a polyhedron
[worked, msg] = run_in_caller('x.setConstraint = 1');
assert(~worked);
...
[worked, msg] = run_in_caller('x.setConstraint = P;');
assert(~worked);
...
[worked, msg] = run_in_caller('x.setConstraint = [P1 P2];');
assert(~worked);
