function test_opt_qp2lcp_39_pass
%
% mpMILP with 1 integer, 2 binaries,  2 continous variable, 2 parameters
% 
...
% S.lb = [1;-1;-5;-17;0];
% S.Ath = [eye(d);-eye(d)];
% S.bth = [13*ones(d,1); 12*ones(d,1)];
% S.vartype = [repmat('C',1,2),'BIB'];

load data_opt_qp2lcp_39
...
for i=1:numel(PU)
    for j=1:PU(i).Num
        ...
        if norm(obj-r.obj,Inf)>MPTOPTIONS.rel_tol
