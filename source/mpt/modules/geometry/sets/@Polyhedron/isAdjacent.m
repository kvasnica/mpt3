...
%
% test if regions P and Q share a common facet
%
% inputs: regions P, Q
...
if ~ts && ( ~isFullDim(P) || ~isFullDim(Q) )
    ...
    if ~isempty(P.H_int)
        [Anp,bnp,Aenp,benp]=mpt_ineq2eq(P.A,P.b);
    else
        Anp=[];bnp=[];Aenp=[];benp=[];
    end
    if ~isempty(Q.H_int)
        [Anq,bnq,Aenq,benq]=mpt_ineq2eq(Q.A,Q.b);
    else
        Anq=[];bnq=[];Aenq=[];benq=[];
    end
        
    % now check if facet i is adjacent to j
    H1 = [Anp, bnp; Aenp, benp; P.He];
    H2 = [Anq, bnq; Aenq, benq; Q.He];
