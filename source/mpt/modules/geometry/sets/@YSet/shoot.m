...
F = obj.constraints + set(obj.vars(:) == x(:)*a);
...
if ismember(d.problem,[12, 15])
    F = F + set( -MPTOPTIONS.infbound*ones(size(obj.vars)) <= obj.vars <= MPTOPTIONS.infbound*ones(size(obj.vars)) );
    F = F + set( -MPTOPTIONS.infbound <= a <= MPTOPTIONS.infbound);
    d = solvesdp(F, -a, obj.opts);
