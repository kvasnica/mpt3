...
% if isfield(U.Internal,'searchTree') 
%     disp('Search tree is already stored in the object, aborting.');
%     return
% end
...
Gi = Fi;
for i=1:U.Num
    Fi{i}=U.Set(i).getFunction('primal').F;
    Gi{i}=U.Set(i).getFunction('primal').g;
end
%Pfinal = bounding_box(ctrlStruct.Pn);
Pfinal = U.Internal.convexHull;
...
for i=1:lenH
    ...
    while (diffrow*diffrow')>MPTOPTIONS.abs_tol;
        ii=ii+1;
        diffrow=[];
        for iii=1:nu
            diffrow=[diffrow [Fi{i}(iii,:) Gi{i}(iii)]-[Fi{ii}(iii,:) Gi{ii}(iii)]];
...
while i<=lenT_Pfinal
    ...
    i=i+1;
    fprintf('Iter %d/%d          \r',i,lenT_Pfinal);
end
...
for i=1:lenT            % all hyperplanes
    ...
    for ii=1:lenH       % all polytopes
        ...
        Kminus=[K; -K2];
        rplus = Polyhedron(Hplus,Kplus).chebyCenter.r;
        rminus = Polyhedron(Hminus,Kminus).chebyCenter.r;        
        ...
        if rplus>MPTOPTIONS.abs_tol  % PA(ii) has elements above hyperplane
            I{2*i}=union(I{2*i},[ii]);
        end 
        if rminus>MPTOPTIONS.abs_tol % PA(ii) has elements below hyperplane
...
% nj is chosen to include all hyperplanes, which minimize the quantites of F
    ...
    for j=1:nj
        ...
        if rplus>MPTOPTIONS.abs_tol    % Pplus is fulldim
            for i=N{Node,1} % faster since only polytops not cut away by earlier hyperplanes have to be checked.
                ...
                rint = Polyhedron([Hplus;Hi],[Kplus;Ki]).chebyCenter.r;
                fulldim = rint > MPTOPTIONS.abs_tol;
                if fulldim  % if yes, add number to index list
                    Iplus{j}=union(Iplus{j},i);
                end
            end
        end
        if rminus>MPTOPTIONS.abs_tol    % Pminus is fulldim
            for i=N{Node,1}
                Hi = Hstore{i};
                Ki = Kstore{i};
                %rint = sub_chebyball([Hminus;Hi],[Kminus;Ki],nx,rel_tol,abs_tol,lpsolver);
                rint = Polyhedron([Hminus;Hi],[Kminus;Ki]).chebyCenter.r;
                fulldim = rint > MPTOPTIONS.abs_tol;
                if fulldim
